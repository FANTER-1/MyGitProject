...
def add_to_all(event):
    ...
    if ip_x < s_x and ip_y < s_y:
        if points[ip_y][ip_x] == -1:
            ...
            if check_winner2():
                print("Победа игрока №2(Корабли противника №1 подбиты)")
    ...
    if ip_x >= s_x + delta_menu_x and ip_x <= s_x + s_x + delta_menu_x and ip_y < s_y:
        ...
        if points2[ip_y][ip_x - s_x - delta_menu_x] == -1:
            ...
            # if check_winner(ip_x, ip_y):
            if check_winner2_player_2():
                print("Победа игрока №1(Корабли противника №2 подбиты)")
                points = [[10 for i in range(s_y)] for i in range(s_x)]
                points2 = [[10 for i in range(s_y)] for i in range(s_x)]
